...
| UpdateMovement | &#54; | Instructs BCS to change the movement for the given section, or to delete it. | [SectionUpdateDTO](<SectionUpdateDTO.md>) | The SectionUpdateDTO holds Tables as an array of TableDTOs. Each TableDTO holds Rounds as an array of RoundDTOs. Each RoundDTO contains data on the opponents and the boards played. Optionally an array of ParticipationDTOs can be added to set the players at the round one tables. The SectionUpdateDTO holds a IsDeleted property that can be set to 'True' if the section must be deleted. If the section does not yet exist, it will be added. |
| AddSession | &#55; | Instructs BCS to add a session to an existing event. | [AddSessionDTO](<AddSessionDTO.md>) | The AddSessionDTO contains a SessionDTO and optionally arrays of PlayerDataDTOs, ParticipationDTOs, HandrecordDTOs and BridgemateSettingsDTOs. |
